...
/***Places to set prefferencial bounds and setting adjustments (ctr+F these terms)
		...
		*	(VRInput) This is where the input is set, Change these for Oculus support:
        * FOR VIVE: Leave as is (VIVE) not commented out, and (OCULUS) commented out
        * FOR OCULUS: Undo comments on all (OCULUS) and comment out (VIVE)
        *
...
*/
public class HandControllerV : MonoBehaviour {



	private GameObject ControllerRoot;		//(VRInput)	(VIVE)
    	
    //public OVRInput.Controller o_controller; //(VRInput) (OCULUS)


    //Reference to Input events for controllers
    private Valve.VR.EVRButtonId gripButton = Valve.VR.EVRButtonId.k_EButton_Grip;				//(VRInput) (VIVE)
	private Valve.VR.EVRButtonId triggerButton = Valve.VR.EVRButtonId.k_EButton_SteamVR_Trigger;//(VRInput) (VIVE)
    //private OVRInput.Button o_gripButton = OVRInput.Button.PrimaryHandTrigger; //(VRInput) (OCULUS)
    //private OVRInput.Button o_triggerButton = OVRInput.Button.PrimaryIndexTrigger; //(VRInput) (OCULUS)
	...
	private SteamVR_Controller.Device controller { get { return SteamVR_Controller.Input((int)trackedObj.index); } }	//(VIVE)
	private SteamVR_TrackedObject trackedObj;																			//(VIVE)

	private Animator animHand;

	private HandControllerV otherHand;		//(VIVE)
//	private HandControllerO otherHand;		//(OCULUS)
	public Transform emptyPrefab;
	private Transform controllerGrabPoint;
	private Transform modelGrabPoint;
...
//	public float normalOfSphere;	//(SetUpHelpers)
	...
	void Awake() {
		...
		//******************(VIVE)*******************************
		ControllerRoot = gameObject.transform.parent.gameObject;
		if (ControllerRoot.name == "Controller (right)"){
			isLeftHand = false;
			otherHand = ControllerRoot.transform.parent.Find ("Controller (left)").Find ("LeftHand").GetComponent<HandControllerV> ();
		}
		else{
			isLeftHand = true;
			otherHand = ControllerRoot.transform.parent.Find ("Controller (right)").Find ("RightHand").GetComponent<HandControllerV>();
		};

		//*****************(OCULUS)******************************
//		ControllerRoot = gameObject.transform.parent.gameObject;
//		if (ControllerRoot.name == "RightHandAnchor"){
//			isLeftHand = false;
//			otherHand = ControllerRoot.transform.parent.Find ("LeftHandAnchor").Find ("LeftHand").GetComponent<HandControllerO> ();
//		}
//		else{
//			isLeftHand = true;
//			otherHand = ControllerRoot.transform.parent.Find ("RightHandAnchor").Find ("RightHand").GetComponent<HandControllerO>();
//		}

		controllerGrabPoint = gameObject.transform.Find ("TargetOffset").transform;
		handOffset = gameObject.transform.Find ("Offsetter").transform;
		modelGrabPoint = handOffset.transform.Find ("GrabPoint").transform;
		animHand = modelGrabPoint.transform.Find ("Hands7HD").gameObject.GetComponent<Animator>();
	}

	void Start() {

        if (ControllerRoot != null)//(VIVE)
        { //(VIVE)

            trackedObj = ControllerRoot.GetComponent<SteamVR_TrackedObject>();		//(VRInput)//(VIVE)
        }//(VIVE)
...
//	}
    ...
    void GetVrInput(){
		gripButtonDown = controller.GetPressDown(gripButton);		//(VRInput) (VIVE)
		triggerButtonDown = controller.GetPressDown(triggerButton); //(VRInput) (VIVE)
        triggerButtonUp = controller.GetPressUp(triggerButton);		//(VRInput) (VIVE)

        //gripButtonDown = OVRInput.GetDown(o_gripButton, o_controller); //(VRInput) (OCULUS)
        //triggerButtonDown = OVRInput.GetDown(o_triggerButton, o_controller); //(VRInput) (OCULUS)
        //triggerButtonUp = OVRInput.GetUp(o_triggerButton, o_controller); //(VRInput) (OCULUS)

        if (triggerButtonUp/*CustomMapping*/){					//used to unclench fist if nothing in hand
            animHand.SetBool("FailGrab", false);
        }

		if (triggerButtonDown/*CustomMapping*/ && !holdingObj && (hoverObj != null)){	//button used to pick up objects
		...
		else if(triggerButtonDown/*CustomMapping*/){					//button used to pick up objects, if none near will make clenched fist
            animHand.SetBool("FailGrab", true);
        }

		if (gripButtonDown/*CustomMapping*/ && holdingObj){			//button used to drop objects
            DropIt();
        }
    }
...
//			grabDistance =  grabToHoldDistance;		//(SetUpHelpers)
	...
	void SetPickup(){		// used in picking up objects to set its position and rotation
		...
		if (heldObj.heldRotation != new Vector3(0, 0, 0)){
			if (isLeftHand) {
				Transform tempTransform = Instantiate (emptyPrefab, heldObj.holdPoint.position, heldObj.transform.rotation) as Transform;
				heldObj.transform.parent = tempTransform;
				tempTransform.parent = modelGrabPoint;
				tempTransform.localEulerAngles = heldObj.leftHeldRotation;
				...
				heldObj.transform.parent = tempTransform;
				tempTransform.parent = modelGrabPoint;
				tempTransform.localEulerAngles = heldObj.heldRotation;
				heldObj.transform.parent = modelGrabPoint;
				Destroy (tempTransform.gameObject);
			}
...
//			else {															//(SetUpLogs)
//				Debug.Log ("angle Y needs to be close to 180 or 360/0");		
//			}
			return false;
